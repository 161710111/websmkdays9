			  	...
			  	<form action="{{ route('ekskul.update',$ekskuls->id) }}" method="post" >
        			...
        			{{ csrf_field() }}
			  		<div class="form-group {{ $errors->has('nama') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" value="{{ $ekskuls->nama }}" name="nama" class="form-control"  required>
			  			@if ($errors->has('nama'))
			  		...
			  		div class="form-group {{ $errors->has('keterangan') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" value="{{ $ekskuls->keterangan }}" name="jadwal" class="form-control"  required>
			  		...
			  		<div class="form-group {{ $errors->has('prestasi_id') ? ' has-error' : '' }}">
			  			...
			  			<select name="prestasi_id" class="form-control">
			  				...
			  				<option value="{{ $data->id }}" {{ $selectedPrestasi == $data->id ? 'selected="selected"' : '' }} >{{ $data->nama }}</option>
			  				@endforeach
			  			</select>
			  			@if ($errors->has('fasilitas_id'))
                            <span class="help-block">
                                <strong>{{ $errors->first('prestasi_id') }}</strong>
                            </span>
                        @endif
			  		</div>
